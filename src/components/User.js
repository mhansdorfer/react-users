...
export default class User extends React.Component {
    ...
    render () {
        ...
        const name = profile.name ? (profile.name.first || "")  + (profile.name.last || "") : "";
        ...
        return (
            <div className='user-profile' style={{backgroundColor: profile.background}}>
                ...
                <div className="info">
                    <h3 className="name"><Link to={`/users/${profile.login.uuid}`}>{name || "Nie przekazano imienia i nazwiska"}</Link></h3>
                    <p className="address">Adres: {address || "Brak"}</p>
